...
static lang_output_section_statement_type *gldelf32btsmip_place_orphan
...
static void
gldelf32btsmip_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32btsmip_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(mips)\n\
ENTRY(__start)\n\
SEARCH_DIR(\"/home/cop4610-18/os161/tools/mips-harvard-os161/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf32btsmip_handle_option (int optc)
{
  switch (optc)
    ...
    case OPTION_AUDIT:
	gldelf32btsmip_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf32btsmip_append_to_separated_string (&depaudit, optarg);
	break;
